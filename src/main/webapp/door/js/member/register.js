...
var validateUsername = (rule, value, callback) => {
  if(!/^(undefined|null|\s)$/.test(value)){
	  ...
	  axios({method: 'get',url: 'door/testuname?username='+vm.member.username}).then(function(response){
...
var validateMobile = (rule, value, callback) => {
  if(!/^(undefined|null|\s)$/.test(value)){
	  axios({method: 'get',url: 'door/checkMobile?mobile='+vm.member.mobile}).then(function(response){
 		   if(response.data.code == 201){
    ...
    methods: {
      submitForm(formName) {
         this.$refs[formName].validate((valid) => {
	          if (valid) {
	            ...
	            axios({method: 'post',url: 'door/subRegister',data: vm.member}).then(function(response){
	      		    if(response.data.code == 200){
	      		    	vm.loading = false;
	      		    	vm.$message({message: "注册成功",duration:500,type: 'success',onClose:function(){
	      		    		window.location.href = "door/toindex.action";
	      		    	}});
					}else{
						vm.loading = false;
      ...
      sendSms:function(){
    	  ...
    	  vm.$refs['member'].validateField('mobile',function(message){
    		  if(message == ""){
    			  axios.get('door/sms?mobile='+vm.member.mobile).then(function(response){
            		  if(response.data.code == 200){
